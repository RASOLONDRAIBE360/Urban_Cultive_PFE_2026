...
class DonneeCapteurService:
    ...
    def create_donnee_capteur(self, choix_parcelle, hum, temp, lum, db_config):
        ...
        if conn and cursor:
            try:
                sql_create = f"INSERT INTO {nouvelle_donnee_capteur.__tablename__} (Id_parc, Humidite, Temperature, Luminosite) VALUES (%s, %s, %s, %s)"
                valeurs = [nouvelle_donnee_capteur.Id_parc, nouvelle_donnee_capteur.Humidite, nouvelle_donnee_capteur.Temperature, nouvelle_donnee_capteur.Luminosite]
    ...
    def select_donnee_capteur(self, choix_parcelle, db_config):
        ...
        if conn and cursor:
            try:
                sql_select = f"SELECT id_parc, humidite, temperature, luminosite, date_pub FROM donnee_capteur WHERE Id_parc=%s"
                ...
                for data_raw in liste_donnee_capteur_brute:
                    ...
                    date_pub = str(data_raw[4])

                    liste_donnee_capteur.append([id_parc, humidite, temperature, luminosite, date_pub])
